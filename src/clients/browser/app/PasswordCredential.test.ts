...
test('Testando se a classe PasswordCredential pode ser instanciada.', async (context) => {
    ...
    await context.test('Definindo um token para a credencial e checando o resultado.', async (subcontext) => {
        ...
        await subcontext.test('Solicitando as informacoes reduzidas da credencial.', () => {
            const info = credentials.info();
            assert.deepEqual(info, { id: 'test@gmail.com', password: 'c0dfa082246e2a2ec22f143d46901665533dde9eeb8170abd6bad6705bd96e2c' })
        })

        await subcontext.test('Solicitando todas as informacoes da credencial.', () => {
